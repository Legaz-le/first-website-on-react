        ...
        <div className="container">
          ...
          >
            {features.map(({ id, icon, caption, title, text, button }) => (
              <div
                key={id}
                className="relative z-2 md:px-10 px-5 md:pb-10 pb-5 flex-50 max-md:g7 max-md:border-2 max-md:border-s3 max-md:rounded-3xl
              max-md:flex-320"
              >
            ...
            >
              ...
              {details.map(({ id, icon, title }) => (
                <li id={id} className="relative pt-16 px-4 pb-14">
                  <div className="absolute top-8 bottom-0 left-1/2 bg-s3/20 w-[1px] h-full z-10" />
                  <div
                    className="flex items-center justify-center mx-auto mb-5 border-2 border-s2 rounded-full hover:border-s4 
